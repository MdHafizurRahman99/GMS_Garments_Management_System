                                ...
                                <div class="form-group">
                                    ...
                                    <select required name="role_id" class="form-control" id="exampleSelect">
                                        ...
                                        @foreach ($roles as $role)
                                            <option {{ $role->id == $role_id ? 'selected' : '' }}
                                                value="{{ $role->id }}">
                                                {{ $role->name }} </option>
                                        @endforeach
                                    </select>
                                            ...
                                            @foreach ($group as $permission)
                                                <div class="form-check m-2 p-2">
                                                    @php
                                                        $i = 0;
                                                    @endphp
                                                    @foreach ($roles_permission as $checkpermission)
                                                        @if ($checkpermission->permission_id == $permission->id)
                                                            @php
                                                                $i = 1;
                                                            @endphp
                                                            <input checked
                                                                ...
                                                                id="name{{ $permission->id }}" name="permission_id[]">
                                                            <label class="form-check-label "
                                                                for="name{{ $permission->id }}">
                                                                {{ $permission->name }}
                                                            </label>
                                                        @endif
                                                    @endforeach
