...
@endif
    ...
    <a href="{{ route('costs.create') }}" class="btn btn-dark mb-4">
        <span class="me-2">+</span> Add Cost
    </a>
    ...
    <form action="{{ route('costs.index') }}" method="GET" class="mb-4">
        <div class="row g-3 align-items-center">
            ...
            <div class="col-auto">
                <label for="type_filter" class="col-form-label">Type:</label>
            </div>
            <div class="col-auto">
                <select name="type_filter" id="type_filter" class="form-select">
                    <option value="All Types" {{ request('type_filter', 'All Types') == 'All Types' ? 'selected' : '' }}>
                        All Types
                    </option>
                    @foreach($expenseTypes as $type)
                        <option value="{{ $type }}" {{ request('type_filter', 'All Types') == $type ? 'selected' : '' }}>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="row mt-4 mb-4">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Total Expenses by Type ({{ date('F', mktime(0, 0, 0, $selectedMonth, 1)) }} {{ $selectedYear }})</h5>
                </div>
                <div class="card-body">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Expense Type</label>
                        <select name="expense_type" id="editExpenseType" class="form-select" required onchange="toggleOtherType(this)">
                            @foreach($expenseTypes as $type)
                                @if($type != 'All Types')
                                    <option value="{{ $type }}">{{ $type }}</option>
                                @endif
